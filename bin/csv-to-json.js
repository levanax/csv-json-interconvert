	...
	function saveFile(dir, fileName, data) {
		fs.writeFile(path.join(dir, fileName), data, function(err) {
			if (err) {
				...
				console.log(chalk.green(dir + '/' + fileName + " file was saved!"));
			}
		});
	}
...
var data = {
	"inputCsvDirectory": config.csv_to_json.csv_file_path,
	"outputJsonDirectory": config.csv_to_json.build_file_path,
	"outputYmlDirectory": config.csv_to_json.build_file_path
}
console.log(config.csv_to_json.build_file_path+'/')
util.checkAndNewDir(config.csv_to_json.build_file_path+'/');

var converter = csv2jsonDirectoryConverter(data);
converter.start();